...
In practice, lack of common support means that some units are too different to be matched. Dropping these units can improve credibility, but it also changes the population for which the treatment effect is estimated. For instance, if only very large firms receive a subsidy and no comparable untreated large firms exist, those treated firms may have to be excluded. The resulting estimate would then apply only to the small and medium-sized treated firms for which comparable untreated units are available, rather than to all treated firms.
