...
Selection-on-observables methods rely on the idea that treatment assignment is non-random, but that the factors driving selection into treatment are observed in the data. If all confounders are observed and measured before treatment,^[A **confounder** is a variable that affects both whether a unit receives the treatment and the outcome it would experience. Because it influences both treatment assignment and potential outcomes, failing to account for it can make treated and untreated units systematically different even before treatment.] differences between treated and untreated units can be adjusted for using pre-treatment covariates. The two main selection-on-observables approaches are regression adjustment and matching. Regression adjustment controls for observed pre-treatment characteristics when comparing outcomes, whereas matching directly compares treated units with untreated units that have similar observed pre-treatment characteristics. Both approaches therefore rely on the same central assumption: after conditioning on observables, no unobserved confounders remain that affect both treatment assignment and potential outcomes.
...
In this model, $\alpha$ is the average outcome for untreated units, while $\tau$ captures the difference in average observed outcomes between treated and untreated units. The term $u_i$ is the error term: it captures all factors affecting the outcome that are not included in the regression.^[Despite its name, the **error term** is not itself an error; rather, it represents the component of the outcome that is not explained by the variables included in the regression model.]
...
With a binary treatment ($D_i$) and an intercept ($\alpha$), the OLS estimate of $\tau$ is equal to the difference in sample means between treated and untreated units:
...
This coefficient is simply the difference in mean outcomes between treated and untreated units. It has a causal interpretation only if this observed comparison reproduces the counterfactual comparison we would like to make between potential outcomes. This requires treatment assignment to be unrelated to potential outcomes, as under random assignment. Therefore, in a properly implemented RCT with full compliance, this simple regression can be used to estimate the average treatment effect.

In observational studies, treated and untreated units often differ for reasons unrelated to the treatment itself. In these cases, the simple regression coefficient $\tau$ does not isolate the causal effect of treatment. Instead, it combines the treatment effect with pre-existing differences between the two groups.
...
where $X_i$ is a vector of observed pre-treatment covariates. Ideally, these covariates should include all confounders, that is, all pre-treatment variables that affect both treatment assignment and potential outcomes.

The timing of the variables is crucial. The covariates in $X_i$ must be measured before treatment occurs, while the outcome $Y_i$ must be measured after treatment. Otherwise, we risk controlling for variables that are themselves affected by the treatment. Doing so can change the interpretation of the regression coefficient and may prevent it from identifying the causal effect of interest.

In this model, $\tau$ compares treated and untreated units **holding constant** the covariates included in $X_i$. The idea is that, after adjusting for relevant observed pre-treatment differences, the remaining difference in post-treatment outcomes may provide a credible estimate of the average causal effect. For this interpretation to hold, several conditions are required.
...
3. There must be sufficient overlap between treated and untreated units. For treated units, there should be comparable untreated units with similar values of the observed covariates. Without common support, the regression relies heavily on extrapolation.^[**Extrapolation** means using the estimated regression relationship to predict outcomes for units whose covariate values are not well represented in the comparison group. These predictions are more model-dependent because they are based on combinations of characteristics for which little or no comparable data are observed.]

4. The regression specification must be credible. OLS is a **parametric** adjustment method: it imposes a functional form for how covariates are related to the outcome. If this functional form is too restrictive, for example because important nonlinearities or interactions are omitted, the estimate may still be misleading. For instance, if the outcome rises sharply with income only above a certain threshold, a simple linear specification in income may adjust treated and untreated units incorrectly and distort the estimated treatment effect.
...
The idea behind matching methods is simple: after treatment assignment has occurred, the control group is constructed **ex post** by selecting untreated units that are similar to treated units in terms of observed pre-treatment characteristics. These matched untreated units are then used to approximate the counterfactual outcomes that treated units would have experienced in the absence of treatment. Unlike in an RCT, therefore, comparability is not created by randomization but by the matching procedure.

Suppose we want to evaluate the effect of a policy $D_i$ on an outcome $Y_i$. As usual, each unit has two potential outcomes:
...
In an observational study, treatment is not randomly assigned. Units may select into treatment, or policymakers may select units for treatment, based on characteristics that also affect the outcome. Matching tries to make treated and untreated units comparable **after the data have been collected**. It does so by comparing treated units only with untreated units that have similar values of observed covariates $X_i$.
...
This means that, conditional on observed covariates $X_i$, treatment assignment is independent of the untreated potential outcome. In words: once we compare treated and untreated units with the same observed characteristics, they are assumed to have had the same average outcome in the absence of treatment.
...
This condition means that, for units with characteristics $X_i=x$, treatment is not certain. In other words, among units with the same, or sufficiently similar, observed characteristics, there must be some untreated units that can serve as comparisons for the treated units. If the probability of receiving treatment is equal to one for a given set of characteristics, matching cannot construct a credible counterfactual for treated units with those characteristics.

In practice, lack of common support means that some treated units have no comparable untreated units. Excluding these units can improve credibility, but it also changes the population for which the treatment effect is estimated. For instance, if firms of all sizes receive a subsidy but no comparable untreated large firms exist, the treated large firms may have to be excluded. The resulting estimate would then represent the ATT among small and medium-sized treated firms with common support, rather than the ATT for all treated firms.
...
In the ideal case, each treated unit would be matched with one or more untreated units that have exactly the same values of the covariates.
...
This expression highlights the core idea of matching: compare treated and untreated units **within comparable covariate cells**, and then average these comparisons over the treated population, giving equal weight to each treated unit.

::: {.callout-note appearance="simple"}
### All treated units receive the same weight
...
The reason is that the ATT asks: what is the average effect for the treated workers? Therefore, each treated worker receives the same weight. Cells with more treated workers contribute more to the final ATT estimate.
:::
...
This probability is not directly observed. What we observe is whether each unit actually received the treatment or not. However, before treatment occurs, we can think of each unit as having some underlying probability of receiving the treatment, depending on its observed characteristics. The propensity score represents this probability and must generally be estimated from the data, for example using a logit or probit model (see below). The resulting estimated propensity score is denoted by $\widehat{p}(X_i)$. Instead of matching units on all covariates $X_i$, PSM matches them on this one-dimensional index.

Rosenbaum and Rubin showed that the propensity score is a **balancing score**: conditional on its true value, treated and untreated units have the same distribution of observed covariates:

$$
X_i \perp D_i \mid p(X_i).
$$

This is the **balancing property** of the propensity score. It does not, by itself, imply balance in unobserved characteristics.

They also showed that, if treatment assignment is independent of potential outcomes conditional on $X_i$, this independence holds conditional on the propensity score:
...
Together with overlap, this result provides the identifying foundation for propensity-score matching. In practice, the propensity score must be estimated, so covariate balance should be checked rather than assumed.
...
In general, logit and probit models are preferred because they ensure that the estimated propensity scores, $\widehat{p}(X_i)$, lie between 0 and 1.
...
Matching therefore usually relies on approximations: untreated units are selected because their propensity score is close to that of the treated unit, not because it is exactly identical. Different matching rules define what “close” means and imply different trade-offs between bias and precision.

::: {.callout-note}
### Bias--precision trade-off

Matching rules differ in how untreated units are selected and weighted to construct the counterfactual for each treated unit, including how many control units are used. Using only the closest control units can reduce bias, because comparisons are made between very similar units, but it may increase variance because little information is used.

Using more control units can improve precision, because the counterfactual is averaged over more observations, but it may increase bias if some of those controls are **more dissimilar from the treated unit**.
...
In what follows, we briefly discuss some of the most common matching rules.
...
4. **Kernel matching.** The counterfactual outcome for each treated unit is constructed as a weighted average of untreated outcomes. Untreated units with propensity scores closer to that of the treated unit receive larger weights, while more distant units receive smaller weights or may receive no weight.
    ...
    The **bandwidth** determines how quickly weights decline as the distance in propensity scores increases.^[In kernel matching, the **bandwidth** controls how broadly control units are used around each treated unit. A smaller bandwidth gives most weight to very close matches, while a larger bandwidth allows more distant controls to receive meaningful weight.]
...
Other related balancing approaches include genetic matching, full matching, and covariate balancing propensity score methods.
...
Matching can improve the comparability of treated and untreated units with respect to **observed pre-treatment characteristics**. However, it cannot balance factors that are unobserved. Therefore, matching identifies a causal effect only if, after conditioning on the observed covariates, there are no remaining unobserved factors that jointly affect treatment assignment and potential outcomes.
:::
