...
For example, a public subsidy may be assigned only to firms whose eligibility score exceeds a given cutoff. Firms just above and just below the cutoff may be very similar in terms of size, productivity, sector, investment plans, and managerial capabilities. However, firms just above the cutoff receive the subsidy, while firms just below do not. The RD design compares these two groups locally around the cutoff.

::: {.callout-note appearance="simple"}
### RD design in one sentence

RD design estimates causal effects by comparing the post-treatment outcomes of units that are very close to a treatment threshold but fall on opposite sides of it.
:::

## Historical Intuition

::: {.content-visible when-format="html"}
![**A gif representation of the sharp RD historical example**.](../images/rdd-intuition-animation.gif)
:::
...
Let $S_i$ denote the running variable for unit $i$, and let $c$ denote the cutoff. Treatment status is denoted by $D_i$. In a **sharp RD design**, treatment assignment is completely determined by the running variable:
...
The central identifying assumption is that potential outcomes evolve smoothly through the cutoff. In other words, absent treatment, there should be no sudden change in the expected outcome exactly at the threshold.
...
The term **local** is essential. The estimate tells us the effect for marginal units whose treatment status changes because they are just above rather than just below the threshold.^[The direction can be reversed: in some RD designs, units below the cutoff are treated and those above it serve as controls. What matters is that treatment status changes discontinuously at the threshold.]
...
where $f(\cdot)$ and $g(\cdot)$ are parametric functions of the centered running variable, specified without intercept terms. The function $f(\cdot)$ describes how the outcome varies with the running variable below the cutoff, while $g(\cdot)$ allows this relationship to differ above the cutoff. Because the running variable is centered at $c$, $\tau_{\text{sharp RD}}$ captures the discontinuity in the conditional mean of the outcome at the threshold.
...
The bandwidth $h$ determines how close units must be to the cutoff to be included in the estimation; units outside this bandwidth are excluded from the estimation sample.
...
Observations closer to the cutoff typically receive more weight, while observations farther away receive less weight. Common choices include triangular and Epanechnikov kernels.
...
::: {.callout-note appearance="simple"}
### The bias–variance trade-off: bandwidth and kernel

An RD estimator must balance two objectives: approximating the outcome relationship accurately near the cutoff and using enough observations to obtain a precise estimate.

**Bias** is a systematic tendency to overestimate or underestimate the true effect. **Variance** describes how much the estimate would change across repeated samples. Ideally, we would like an estimator with both zero bias and zero variance, but in practice this is unattainable. Instead, estimation often involves a **bias–variance trade-off**: reducing bias may come at the cost of greater variance, while reducing variance may require accepting some bias.

The **bandwidth** determines the estimation window around the cutoff:
- A **smaller bandwidth** uses observations closer to the threshold. Over this narrower range, a local linear approximation is generally more credible, reducing approximation bias. However, fewer observations are available, so estimates tend to be less precise.
- A **larger bandwidth** includes more observations and generally reduces variance. However, if the outcome relationship is curved, fitting a straight line over a wider range may poorly approximate its value at the cutoff, increasing bias.

For example, with a test-score cutoff of 70, using scores between 69 and 71 provides a very local comparison but may leave few students. Using scores between 60 and 80 provides more data, but requires the fitted lines to approximate the outcome relationship over a much wider range.

The **kernel** determines how observations within the bandwidth are weighted. A uniform kernel assigns equal weight to all included observations, whereas a triangular kernel gives more weight to observations closer to the cutoff and progressively less weight to those farther away. Different kernels therefore handle the bias–variance trade-off differently, although the choice of bandwidth is typically more consequential.

Modern RD methods combine data-driven bandwidth selection with **robust bias-corrected confidence intervals**, which account for the uncertainty introduced by estimating and correcting approximation bias [@calonico2014]. These tools improve estimation and inference, but cannot repair a design whose identifying assumptions fail.
:::
...
The visual evidence is not a substitute for formal estimation, but it is informative. A clear discontinuity in the outcome at the cutoff, combined with the absence of similar discontinuities in predetermined covariates, provides convincing supporting evidence for a causal interpretation of the RD estimate.
:::
...
![Stylized representation of the RD application reported in Lee (2008).](../images/sharp_RD_example_Lee.png){width=80%}
...
Lee estimates the effect of party incumbency on electoral performance in the next election. The results reveal a substantial incumbency advantage: barely winning the current election increases the probability that the same party wins the district in the next election by about 40 percentage points and raises its subsequent vote share by roughly 8 percentage points.

## Fuzzy RD design

The discussion so far has focused on **sharp RD**, the simplest version of the design. In a sharp RD, crossing the cutoff determines treatment status perfectly: units on one side of the threshold are treated, while those on the other side are untreated. In many real-world applications, however, crossing the cutoff only changes the **probability** of receiving treatment rather than treatment status itself. This setting is known as **fuzzy RD**.
...
The numerator measures the jump in the outcome at the cutoff, while the denominator measures the jump in the probability of treatment. The fuzzy RD estimand is therefore a ratio: it scales the discontinuity in the outcome by the discontinuity in treatment receipt. For example, suppose crossing the cutoff increases the average outcome by 4 units and raises the probability of receiving treatment from 30% to 70%. The treatment probability therefore increases by 0.40, so the fuzzy RD estimate is $$\frac{4}{0.40}=10.$$ This means that treatment increases the outcome by 10 units, on average, for the units near the cutoff whose treatment status is changed by crossing the threshold.

For this reason, **fuzzy RD is closely related to instrumental variables**. In a fuzzy RD design, the indicator for being above the cutoff can be used as an instrument for actual treatment receipt. The logic is the same as in an IV design. The cutoff generates a source of exogenous variation in treatment: units just below and just above the threshold are assumed to be comparable, but those just above the cutoff are more likely to receive the treatment. Under the usual RD continuity assumptions, and under the IV assumptions of relevance, exclusion, and monotonicity, the fuzzy RD estimand identifies a **LATE** for units close to the cutoff whose treatment status is changed by crossing the threshold.
...
Spatial RD is attractive because nearby locations often share many observed and unobserved characteristics. However, it has additional challenges.

First, several policies may change at the same boundary. If multiple institutional rules differ across the border, the observed discontinuity may capture the combined effect of these differences rather than isolate the policy of interest. This is sometimes called a **compound treatment** problem. To interpret the discontinuity as the effect of a specific policy, researchers must therefore impose an additional assumption, often referred to as **compound treatment irrelevance**: the other policies or institutional differences that also change at the boundary must not themselves generate a discontinuity in the outcome. An alternative, when the policy of interest is introduced at a known point in time along an already existing boundary, is to combine spatial RD with DiD. By comparing the discontinuity before and after the policy change, this approach can difference out pre-existing border differences, provided that these differences would have remained stable over time in the absence of the policy.
...
The policy targeted municipalities located within a certain distance of the inner-German border. The policy was introduced to compensate municipalities close to the inner-German border for the economic disadvantages created by the division of Germany. The new border abruptly turned these areas into peripheral locations, cutting them off from former markets and economic connections and weakening their development prospects. The geographic eligibility rule, which defined the *Zonenrandgebiet* as an approximately 40-km-wide band along the inner-German border, generated a discontinuity in treatment assignment at the boundary of the assisted area: municipalities just inside the eligible border zone received support, while nearby municipalities just outside did not. The key idea of the design is therefore simple. Municipalities located very close to the same boundary are likely to be similar in many respects, except for their eligibility status. This makes it possible to compare units on opposite sides of the border and interpret discontinuous differences in outcomes as evidence of a treatment effect.

The figure below illustrates the geographic discontinuity exploited in the study. The authors find that the subsidies produced persistent increases in local economic activity, even after the program ended, although part of these gains appears to reflect the relocation of economic activity from nearby untreated areas.

![Stylized representation of the spatial discontinuity in the West-German *Zonenrandgebiet*. Municipalities just inside the eligible border zone were exposed to the policy, while nearby West-German municipalities just outside were not.](../images/spatial_RD_example_Seidel.png){width=70%}

## Summary

An RD design exploits a discontinuous treatment rule. If units cannot precisely manipulate the running variable and potential outcomes vary smoothly at the cutoff, units just below and just above the threshold are expected to be similar in all respects except treatment status.
