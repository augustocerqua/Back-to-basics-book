...
The methods discussed so far rely on the availability of untreated units that can be used to construct a credible counterfactual. However, in many policy settings, such units may be unavailable or may not provide a credible comparison group. For instance, this may occur when:

i) **The treatment affects all units at the same time.** This can happen with nationwide reforms, macroeconomic shocks, pandemics, or policies implemented simultaneously across all units. In such settings, no untreated units may remain in the data.
...
The logic is simple: if no credible control group is available, we can use the pre-treatment information of the treated units themselves to forecast the missing counterfactual.^[In some settings, pre-treatment information from untreated units can also be used to improve the forecast, even if those units cannot provide a credible post-treatment comparison group.]
...
where $\mathcal{H}_{T_0}$ is the information available before treatment. This information may include pre-treatment values of the outcome and other covariates, time-invariant characteristics, seasonal patterns, and underlying trends.

The treatment effect is then the gap between the observed post-treatment outcome and the predicted untreated path.

The logic is intuitive: the pre-treatment evolution of the outcome is used to predict what would have happened after treatment in its absence. Any systematic departure from this predicted path can then be interpreted as evidence of a treatment effect, provided the forecasting assumptions are credible (see Section 11.7).

For example, suppose employment in a country had been increasing steadily before a reform. If employment rises substantially more than predicted after the reform, the additional increase may be attributed to the treatment. A similar logic is used to measure excess mortality. During a pandemic, for instance, mortality observed in a region can be compared with the baseline level predicted from pre-pandemic data; the difference is interpreted as excess mortality.
...
The point effect at post-treatment time $t$, with $t > T_0$ is
...
The same logic can also be used to study treatment-effect heterogeneity by comparing the estimated effects across different subgroups of treated units.
...
The **interrupted time-series (ITS) design** is probably the most widely used FBCM. It is particularly useful when there is a single treated unit and a sufficiently long pre-treatment time series is available to model its underlying trajectory.

The idea is to model the pre-treatment dynamics of the outcome and then extrapolate them into the post-treatment period.

A simple ITS model can be written as

$$
Y_t=\alpha+\beta t+\gamma Post_t+\delta(t-T_0-1)Post_t+u_t,
$$

where $T_0$ denotes the last pre-treatment period and $Post_t=\mathbf{1}(t>T_0)$. The intervention therefore starts at $T_0+1$. In this specification, $\beta$ captures the pre-intervention trend, $\gamma$ captures the immediate level change relative to the counterfactual trend in the first treated period, and $\delta$ captures the change in trend after the intervention. The term $(t-T_0-1)Post_t$ equals zero in the first treated period, so the intervention-related change at that point is $\gamma$. Thereafter, the post-intervention trend is $\beta+\delta$.

This representation is useful pedagogically, but real applications often require more careful modeling. Outcomes may be autocorrelated, seasonal, volatile, or affected by calendar effects. For this reason, ITS analysis is often combined with time-series models such as ARIMA.
...
![ITS logic applied to the Italian smoking ban. The post-policy effect is estimated by comparing observed hospital admissions with a forecasted counterfactual trajectory based on pre-policy data.](../images/italian-smoking-ban-its-stylized.png){width="90%"}

Since all units in Italy were treated simultaneously, there is no obvious untreated group within the country that can be used to construct the counterfactual. One possibility would be to use similar data from other countries that did not introduce the same policy at the same time. However, this strategy would be credible only if other countries provided a suitable comparison for Italy and hospital admission data were measured in a sufficiently comparable way across countries.
...
The key contribution is not simply the use of ARIMA as a forecasting method. Rather, the method embeds forecasting within a causal framework: it defines causal estimands using potential outcomes, clarifies the assumptions under which deviations from the forecast can be interpreted as treatment effects, and provides a framework for statistical inference.

The basic idea can be summarized in four steps:

i) estimate the untreated time-series process using pre-treatment data;
ii) forecast how the outcome would have evolved after treatment in its absence;
iii) compare the observed post-treatment outcomes with this forecast to estimate treatment effects;
iv) quantify the uncertainty surrounding these estimates.

For example, Causal-ARIMA has been used by [@menchetti2023] to estimate the effect of a permanent price reduction on sales in a retail setting where only the treated product series is available. The treatment effect is estimated as the difference between observed post-treatment sales and the sales that would have been predicted in the absence of the price reduction.


::: {.callout-note}
### Confidence intervals and prediction intervals

A **confidence interval** describes the uncertainty around an estimated parameter or expected outcome, such as an average treatment effect. It asks: how precisely have we estimated the quantity of interest?

```{=latex}
\smallskip
```

A **prediction interval** describes the uncertainty around an individual future or unobserved outcome. It accounts not only for uncertainty in the estimated model, but also for the variability of the outcome around its predicted mean. It asks: how uncertain is the outcome we are trying to predict?

```{=latex}
\smallskip
```

This distinction is particularly relevant for counterfactual methods. In CBCMs, such as DiD or matching, the main object is usually a causal parameter obtained by comparing treated units with observed control units. Inference therefore typically focuses on uncertainty around the estimated treatment effect, which is reported using confidence intervals.

```{=latex}
\smallskip
```

In FBCMs, the counterfactual is forecast from the treated unit's own pre-treatment history rather than observed in a comparison group. If the object of interest is the specific counterfactual outcome that would have occurred, uncertainty concerns an unobserved outcome itself, making a prediction interval a natural inferential object. If instead the object of interest is the expected counterfactual outcome or an average treatment effect, confidence intervals may also be appropriate.
:::
...
1) **Stability of the untreated outcome process**. In the absence of treatment, the relationship learned from pre-treatment data should continue to hold in the post-treatment period. If the data-generating process changes for reasons unrelated to the policy, the forecast error may capture those changes rather than the treatment effect.

2) **Absence of other simultaneous shocks**. If another event that could itself affect the treated units’ outcome occurs at the same time as the treatment, it may be impossible to disentangle the effect of the treatment from that of the concurrent event.

3. **Sufficient pre-treatment information**. Forecasting requires *enough* historical data to learn the relevant pattern.^[There is no universal minimum number of pre-treatment periods. What counts as *enough* depends on the complexity and stability of the outcome process: learning trends, seasonality, or other recurring patterns generally requires a longer pre-treatment series than learning a simple and stable trajectory.]

4) **No anticipation**. Units should not change behavior before the start of the intervention because they expect the treatment to occur. Anticipation contaminates the pre-treatment period and makes the estimated untreated trajectory unreliable. This assumption also underlies most CBCMs.

::: {.callout-warning}
### Machine learning and time-series models do not remove the causal problem

Machine learning, time-series models, or a combination of the two can improve the accuracy of the counterfactual forecast, but accurate prediction alone is not enough for causal inference. If another event affects the treated units’ outcome after treatment, the observed deviation from the forecast may partly reflect this event rather than the treatment effect of interest.
:::
...
- inspecting forecast errors during the pre-treatment period;
- comparing the placebo performance of the chosen method with simple forecasting benchmarks, such as the mean, naive, and drift methods;
- checking whether the results are driven by a few influential periods;
...
![**A gif representation of the MLCM worked example**.](../images/forecast-based-workplace-safety-animation.gif)
:::
...
::: {.callout-note}
### Combining forecast- and control-based information: controlled ITS

A **controlled interrupted time-series (controlled ITS)** design combines elements of forecast-based and control-based counterfactual methods. As in a standard ITS, the pre-intervention trajectory of the treated series is used to project how its outcome would have evolved after the intervention. At the same time, one or more untreated series are used to capture contemporaneous shocks or changes that are unrelated to the intervention.

```{=latex}
\smallskip
```

The causal effect is therefore inferred from whether the treated series changes at the intervention date **relative to both its own previous trajectory and the change observed in the control series**. Compared with an uncontrolled ITS, this can make the counterfactual more credible when treated and control series would have been affected similarly by other events occurring during the study period [@lopezbernal2018controls].

```{=latex}
\smallskip
```

This advantage depends on the choice of controls. The control series should have comparable pre-intervention dynamics, should be exposed to the same relevant external shocks, and should not itself be affected by the intervention. A poorly chosen control can weaken rather than strengthen the design.
:::
...
The simplest versions use mean, naive, or drift forecasts. More sophisticated approaches include ITS designs and the MLCM.
