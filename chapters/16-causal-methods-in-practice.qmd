...
|:------------------------|:----------------------------------|---------:|:----------------------:|------------:|
| Linear OLS              | All pupils (treated and controls) |     4.85 |       [4.34, 5.37]     |        4.00 |
| $\rule{0pt}{0.2em}$     |                        |          |                        |             |
| PSM with a 0.05 caliper | Matched treated pupils and controls |     3.98 |       [3.52, 4.44]     |        4.00 |
...
$$
\widehat{\operatorname{LATE}}_{\text{Wald}}
=
\frac{
\overline{Y}_{Z=1}-\overline{Y}_{Z=0}
}{
\overline{D}_{Z=1}-\overline{D}_{Z=0}
}
=
\frac{\widehat{\rho}_1}{\widehat{\pi}_1},
$$

where $\overline{Y}_{Z=z}$ is the average observed earnings of individuals with instrument value $Z_i=z$, and $\overline{D}_{Z=z}$ is the proportion of individuals with $Z_i=z$ who served in the military.
...
3. **random forest**, estimated by cross-validating over three values of `mtry`: all covariates, half of the covariates, and one third of the covariates.^[MLCM can accommodate a wide range of predictive methods. For brevity, here we illustrate it using only two: LASSO and random forest.]
...
Averaging across municipalities with equal weights, the true simulated excess mortality is 7.920 deaths per 10,000 inhabitants, while MLCM estimates 8.172. These quantities describe the effect for the average municipality, not the population-weighted national excess-mortality rate.^[The population-weighted national excess-mortality rate would require weighting each municipality by its population, or aggregating deaths and population directly.]
