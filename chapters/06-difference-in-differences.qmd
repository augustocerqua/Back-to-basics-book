...
The target parameter is usually the **ATT** in the post-treatment period:
...
An early example of DiD logic can be found in John Snow's analysis of cholera in nineteenth-century London [@snow1855]. At the time, cholera was often explained by the dominant miasma theory, according to which disease spread through “bad air.” Snow instead argued that cholera was transmitted through contaminated water, but he needed empirical evidence to support his theory.

The empirical setting was unusually informative. In parts of London, households were served by different water companies. Before the relevant change, both the Lambeth Company and the Southwark & Vauxhall Company drew water from polluted sections of the River Thames. Later, Lambeth moved its water intake upstream,^[Lambeth had moved its water intake upstream in 1852 to improve water quality, relocating it beyond the main influence of London’s sewage. This change provided the crucial variation Snow exploited to test the waterborne-transmission hypothesis.] to a cleaner part of the river, while Southwark & Vauxhall continued to draw water from a more contaminated downstream source.
...
![John Snow’s study of cholera in London provides an early illustration of DiD logic. The numbers reported in the bottom-left figure are not Snow’s original data; they are purely illustrative and serve only to make the DiD comparison transparent.](../images/snow_DID_example.png){width=85%}
...
The relevant assumption then becomes **conditional parallel trends**: after conditioning on pre-treatment characteristics, the treated and control groups would have followed the same average trend in $Y$ in the absence of treatment.
...
This is why the choice between levels, logs, rates, and other transformations should be substantive rather than mechanical, and ideally made before examining the results to avoid cherry-picking the specification that produces the most favorable estimate.
...
With several pre-treatment periods, researchers can estimate placebo DiD effects using periods before the treatment actually occurred. If the method finds large **pre-treatment placebo estimates**, this suggests that the design may be capturing pre-existing differences in trends rather than the causal effect of the treatment.

Other useful checks include:

- plotting average outcomes for treated and control groups over time, rather than only the differences between them;
- checking sensitivity to alternative comparison groups;
- checking whether the estimated effect changes when adding relevant pre-treatment covariates;
- checking whether other policies or shocks occurred at the same time.

::: {.callout-caution appearance="simple"}
### Pre-trends are informative, not definitive

Similar pre-treatment trends support the credibility of a DiD design, but they do not prove that counterfactual trends would have remained parallel after treatment, since this assumption is not fully testable. Conversely, diverging pre-treatment trends should be viewed as a strong warning sign.
:::
...
Suppose workers enter a training program after a sudden fall in earnings. Even without the program, their earnings might partially of fully recover. A DiD estimator comparing these workers with non-participants may mistakenly attribute the natural rebound to the training program.
...
Triple differences do not eliminate the need for identifying assumptions. Instead, they require a **parallel-trends assumption for the difference between two DiD comparisons**: in the absence of the policy, the gap between these two DiD effects would have remained unchanged over time. Under this assumption, which is not fully testable, any post-treatment change in this gap can be attributed to the policy-specific effect of interest.

## Matching Difference-in-Differences

Matching DiD combines the logic of matching with the logic of DiD. It is useful when treated and control units differ in observed characteristics, but panel or repeated cross-sectional data are available.^[**Repeated cross-sectional data** consist of separate samples drawn from the same population at different points in time. Unlike panel data, the same units do not need to be observed in every period.]
...
In the canonical DiD setting, there are two groups and two periods: one group becomes treated, the other remains untreated, and the DiD estimand compares the change in outcomes for the treated group with the change in outcomes for the control group. In such a simple setting, the DiD parameter has a transparent interpretation: it estimates the ATT under the parallel-trends assumption.

However, many empirical applications are more complex. Units may adopt the treatment at different times: some are treated early, others later, and some may never be treated. This setting is known as **staggered adoption**. In this case, the causal effects of interest are often **group-time average treatment effects**, usually denoted as $ATT(g,t)$. These parameters measure the average effect of the treatment at time $t$ for the group of units first treated in period $g$.
...
Here, $\alpha_i$ are unit fixed effects, which absorb time-invariant differences across units. The terms $\lambda_t$ are time fixed effects, which absorb shocks common to all units in a given period.^[In practice, **fixed effects** can be thought of as a set of dummy variables. Unit fixed effects include a separate indicator for each unit, while time fixed effects include a separate indicator for each period. These dummies absorb persistent differences across units and common differences across periods.] The variable $D_{it}$ indicates whether unit $i$ is treated at time $t$. The coefficient $\tau$ is often interpreted as the ATT.


::: {.callout-note appearance="simple"}
### Application: unilateral divorce laws and dynamic effects

Wolfers [-@wolfers2006] studies whether the staggered adoption of **unilateral divorce laws** raised divorce rates in the United States. Between 1968 and 1988, 29 states changed their divorce laws at different times. A natural static TWFE specification is
...
where state fixed effects $\alpha_s$ absorb permanent differences across states, year fixed effects $\lambda_t$ absorb common national shocks, and $Unilateral_{st}$ indicates whether unilateral divorce was available in state $s$ and year $t$. The coefficient $\tau$ compresses the effects observed at all post-reform horizons into a single estimate.

This restriction is consequential because the effect was not constant over time. In Wolfers's dynamic specification, divorce rates increased by approximately **0.27 divorces per 1,000 people** during the first two years after reform and by **0.22** during years three and four. The estimated increase then became smaller, and by years nine and ten the estimate was approximately **-0.10**, providing no evidence of a persistent positive effect.
:::

However, in staggered-adoption designs, $\tau$ does not generally estimate a single traditional DiD effect. It is instead a weighted average of many underlying comparisons across groups and time periods. Some of these comparisons use not-yet-treated units as controls, which is often natural. Others use already-treated units as controls for newly treated units. These comparisons can be problematic when treatment effects are heterogeneous, and especially when they evolve over time after treatment, because already-treated units are themselves experiencing treatment effects while serving as controls.

::: {.callout-warning}
### Why TWFE needs caution

With staggered treatment timing and heterogeneous treatment effects, a TWFE coefficient may not estimate the policy-relevant ATT.
:::
...
The problem appears after both groups have been treated. Once the earlier-treated group has already been exposed to the policy, its outcomes may include treatment effects. UUsing this group as a control for the later-treated group means comparing treated units with other treated units, rather than with units that remain untreated throughout the analyzed period. These comparisons are often called **forbidden comparisons**. They are problematic because they may subtract part of the treatment effect from the estimate, especially when treatment effects grow, fade, or otherwise evolve over time. In that case, the comparison no longer isolates the untreated potential outcome trend.
:::

In recent years, several estimators have been proposed as alternatives to the standard TWFE model, especially in settings with staggered treatment adoption and heterogeneous treatment effects. These approaches are often discussed within the broader **event-study** literature, because they estimate how treatment effects evolve before and after treatment adoption while avoiding some of the problematic comparisons embedded in TWFE regressions. We will discuss these methods in ([Chapter 7.](../chapters/07-event-studies.qmd))
...
In the simple two-group, two-period case, DiD is transparent and intuitive. In richer panel settings with staggered adoption and heterogeneous treatment effects, researchers must be more careful, as detailed in the next chapter.
