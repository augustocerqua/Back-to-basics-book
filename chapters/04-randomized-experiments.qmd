...
![**The balancing power of randomization.** Random assignment makes treated and control units comparable in expectation: when the sample size is sufficiently large, pre-treatment characteristics should be balanced across groups on average.](../images/rct-randomization-balance-animation-v2.gif)^[This is a stylized example in which balance is perfect with $N=500$. In practice, with finite samples, we should expect some small differences between treated and control groups, although these differences tend to decrease as $N$ increases.]
:::

::: {.content-visible when-format="pdf"}
![**The balancing power of randomization.** Random assignment makes treated and control units comparable in expectation: when the sample size is sufficiently large, pre-treatment characteristics should be balanced across groups on average.](../images/rct-randomization-balance-three-phases.png)^[This is a stylized example in which balance is perfect with $N=500$. In practice, with finite samples, we should expect some small differences between treated and control groups, although these differences tend to decrease as $N$ increases.]
::: 

## What is an RCT?

A **randomized controlled trial (RCT)** is the benchmark research design for causal inference. It is so central to the logic of causal analysis that we have already briefly discussed it in each of the previous chapters. Its logic is simple: if treatment is assigned randomly, treated and untreated units are comparable in expectation before the intervention. Therefore, under a properly implemented RCT, systematic differences in outcomes observed after the intervention can be interpreted as causal effects of the treatment rather than as the result of pre-existing differences between the two groups.
...
Because treatment is randomly assigned,^[**Identification vs. estimation.** **Identification** is a population statement: under random assignment, the relevant population averages are exactly equal to the corresponding potential-outcome averages. **Estimation** uses a finite sample to approximate these population quantities, so the corresponding sample averages will generally be only approximately equal because of finite-sample variation. In what follows, we show the key identification result implied by random assignment.]
...
At the same time, Project STAR also shows that even large RCTs require careful interpretation. Researchers must consider issues such as compliance, attrition, implementation quality (see Section 4.4), and whether the results can be generalized to other schools, states, or education systems (see Section 4.5).
:::
...
The key point is that random assignment makes the two groups highly similar on average before they are exposed to the webpage. Therefore, if version B produces a higher conversion rate, the difference can be attributed to the change being tested, rather than to pre-existing differences between users.
...
Although randomized experiments are the gold standard in causal inference, random assignment alone does not automatically guarantee a valid causal estimate. Several problems can threaten the **internal validity** of an RCT:^[**Internal validity** refers to the credibility of the causal conclusion within the study: whether the observed difference in outcomes can truly be attributed to the treatment rather than to other factors.]
...
$$
  ...
  where $W_i$ denotes assignment to treatment. The ITT measures the effect of being offered or assigned the treatment, regardless of whether units actually comply. This can be highly relevant for policy, because real-world programs often face imperfect take-up, but it should not be confused with the average effect of actually receiving the treatment.
...
For example, suppose students with lower pre-treatment achievement are more likely to drop out of a schooling experiment. If this selection process is the same in the treatment and control groups and is not itself affected by treatment assignment, randomization can still preserve comparability among the students who remain in the sample. The estimated effect can then remain unbiased for this selected subpopulation, although it no longer necessarily represents the effect for the original experimental population. By contrast, if dropout depends differently on students’ characteristics across the treatment and control groups, or is itself affected by treatment, comparability may be lost and the resulting estimate may suffer from attrition bias.
...
Beyond these threats to internal validity, RCTs may also be **infeasible for ethical or economic reasons**. Ethical constraints can make random assignment unacceptable. For example, it would be ethically problematic to randomly deny emergency food assistance to some eligible households after a natural disaster simply to create a control group. RCTs can also be very costly, as they may require recruiting participants, implementing the intervention, monitoring compliance, and collecting outcomes over time. Thus, even when randomization would be desirable from an identification perspective, ethical or financial constraints may prevent its use and make observational or quasi-experimental approaches necessary.

## Threats to External Validity

An experiment may identify the causal effect for a particular population, period, and implementation setting. However, the estimated effect may differ for other populations, periods, or implementation settings.
...
This is the issue of **external validity**: whether the estimated effect generalizes beyond the experimental setting.^[**External validity** refers to the extent to which a causal effect estimated in a particular study can be expected to apply to other populations, periods, or implementation settings.]
...
External validity is a central issue in medical RCTs. A drug may be shown to be effective and safe for the population included in a trial, but this does not automatically imply that the same conclusion applies to all potential patients.
