...
This binary representation is useful and often realistic. Policies may be organized around access—for example, receiving the program, being eligible for a benefit, or being exposed to a reform. In other settings, however, treatment is not simply a yes-or-no variable. Units may receive different **amounts**, **intensities**, or **doses** of the same policy.
...
![With a continuous treatment, a unit can receive many possible treatment intensities. In this example, each possible subsidy amount corresponds to a different potential outcome. For each firm, we observe only the outcome associated with the realized subsidy amount; the outcomes under all other possible amounts remain counterfactual.](../images/firm-subsidies-continuous-treatment.png)
...
The relevant support of the treatment matters because causal effects can be studied only over treatment intensities that are meaningfully represented in the data. Extrapolating to doses that are rarely observed, or not observed at all, requires much stronger assumptions that are also inherently untestable.
:::
...
**ADRF** describes how the average potential outcome would change across different levels of treatment intensity. **For each possible treatment level, it gives the average outcome that would be observed if all units were assigned that specific intensity.**
:::
...
With a continuous treatment, we may instead ask:

> What is the effect of receiving a larger dose of the policy? Is there an optimal level of treatment intensity?

This distinction has direct implications for policy design. If the ADRF flattens beyond a certain treatment intensity, allocating additional resources may yield only limited marginal returns. By contrast, if the ADRF continues to rise steeply, increasing treatment intensity may generate substantial additional benefits.

::: {.callout-note appearance="simple"}
### Beyond continuous treatment intensity

This chapter focuses on settings in which treatment varies **continuously in intensity**. A related challenge arises when treatment is **discrete but has multiple versions**. For instance, unemployed workers may participate in different active labor market programs, such as training, job-search assistance, or wage subsidies. In this setting, the relevant causal question is not simply whether receiving a program has an effect, but how the effects differ across alternative treatments.

The methods required for multiple discrete treatments differ from those used for continuous treatment intensity, even though both settings move beyond the standard binary treatment framework.^[Methods for multiple discrete treatments are not discussed in this book]
:::

## The Challenge of Comparing Treatment Intensities

Continuous-treatment evaluation is difficult because treatment intensity is generally endogenous. Only when treatment intensity is randomly assigned can units receiving different doses be directly compared without further adjustment.

In most policies, however, the dose is not randomly assigned. Firms receiving larger subsidies may have larger projects, stronger administrative capacity, better political connections, or more severe constraints. Regions receiving more funds may differ systematically from those receiving less, as in the EU Cohesion Policy example. Patients receiving higher drug doses may have more severe baseline conditions.
...
This is why, in observational studies, estimating an ADRF is not simply a matter of plotting outcomes against treatment intensity. A raw regression of $Y_i$ on $T_i$ compares different populations at different treatment levels. As a result, it may capture selection into dose rather than the causal effect of dose.

## The Generalized Propensity Score

The propensity score was originally introduced for binary treatments [@rosenbaum1983], as shown in [Chapter 5](../chapters/05-selection-on-observables.qmd). Hirano and Imbens extended this idea to continuous treatments through the **generalized propensity score**, or **GPS** [@hirano2004].
...
The GPS summarizes the likelihood of observing a given treatment intensity $T_i$ for a unit with characteristics $X_i$, as captured by the conditional density of treatment intensity given covariates.
...
Averaging these predicted potential outcomes across units gives the estimated ADRF:
...
::: {.callout-note appearance="simple"}
### Two steps of the GPS approach

1. Model treatment intensity as a function of pre-treatment covariates and estimate the GPS.
2. Use the estimated GPS to adjust for differences across treatment intensities and recover the dose-response function.
:::

## Identification Assumptions

### Weak Unconfoundedness With Continuous Treatments
...
In practical terms, this means that we need comparable units receiving different treatment intensities. For example, if all small firms receive low subsidies and all large firms receive high subsidies, it is generally impossible to disentangle the effect of subsidy intensity from the effect of firm size.
...
![This is a stylized version of the ADRF reported in the paper. It illustrates the intuition behind the ADRF: how the expected outcome changes as exposure intensity increases, after adjusting for observed confounders through GPS matching. Under the assumptions required for GPS matching, the upward-sloping curve indicates that higher long-term PM2.5 exposure causally increases relative mortality risk.](../images/wu_pm25_adrf_stylized_slim.png)
:::
...
1. **Inspect the distribution of treatment intensity.** If most observations are concentrated in a narrow range, the dose-response function outside that range will have limited empirical support and rely heavily on extrapolation.

2. **Assess common support.** The analysis should show whether comparable units exist at different treatment levels.

3. **Check covariate balance across the treatment distribution.** The goal is to reduce the association between treatment intensity and pre-treatment covariates.

4. **Report the ADRF with uncertainty bands.** The curve may look smooth, but estimates at extreme treatment levels may be very imprecise.

5. **Test sensitivity to alternative specifications.** Researchers should assess robustness to alternative treatment models, outcome models, and dose ranges.
...
1. **Do not convert a continuous treatment into a binary treatment without a substantive reason.** This may hide important variation in the relationship between treatment intensity and the outcome.

2. **Do not interpret a raw regression of outcomes on treatment intensity as causal.** In observational studies, units receiving different doses may differ systematically in their potential outcomes.

3. **Do not estimate the dose-response function in regions with limited empirical support.** This may turn heavy extrapolation into seemingly causal evidence.

4. **Do not focus only on whether the curve is increasing or decreasing without considering uncertainty.** Apparent nonlinearities may simply reflect statistical noise.

5. **Do not ignore the relevant policy margin.** The key question is often not the effect of moving from zero to a very high dose, but the effect of changing treatment intensity within the range over which policymakers can realistically intervene.
...
If the ADRF declines sharply at low subsidy levels and then flattens, this suggests that modest subsidies may generate most of the safety gains. If the function continues to decline at higher doses, larger subsidies may be justified. If the curve becomes flat or even increases, additional funding may be inefficient.
...
A useful alternative is to estimate dose-response functions separately for theoretically defined or data-driven subgroups, as in the Clustered Dose-Response Function approach [@cerqua2026cldrf]. The target then shifts from one population-average curve to a set of subgroup-specific curves, which may be more informative for policy design. However, creating more homogeneous groups does not automatically make the unconfoundedness assumption less demanding. Within each subgroup, treatment intensity may still be related to unobserved determinants of the outcome. A causal interpretation therefore still requires a credible justification for unconfoundedness and sufficient overlap over the doses being compared.
:::
