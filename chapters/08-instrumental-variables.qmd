...
In observational studies, treated and untreated units often differ in ways that are not fully observed by the researcher. If unobserved factors affect both treatment participation and potential outcomes, a regression of the outcome on treatment does not generally identify a causal effect, even after controlling for observed pre-treatment differences.
...
This means that treatment status is correlated with unobserved determinants of the outcome. In this case, estimating $\tau$ by ordinary least squares does not generally yield an unbiased estimate of the causal effect of $D_i$ on $Y_i$.
...
An early and influential example of IV logic appears in Philip G. Wright's analysis of supply and demand in agricultural commodity markets [@wright1928]. Wright faced the problem of estimating a demand curve when price and quantity are jointly determined in market equilibrium. A simple regression of quantity on price cannot generally recover the demand relationship, because observed changes in prices and quantities may result from shifts in either demand or supply.

The key insight was to exploit variables that shift supply without directly shifting demand. Weather conditions, for example, may affect agricultural output and therefore supply, while leaving consumers' demand unchanged. These supply shifts generate changes in equilibrium prices that move the market along the demand curve, making it possible to estimate how quantity demanded responds to price. In modern terminology, weather-related supply shifters can therefore serve as **instruments** for price: they generate changes in price that originate from the supply side rather than from changes in consumers' demand. By isolating this source of price variation, we can study how quantity demanded responds to changes in price.

The Wright example captures the core logic of IV: when treatment is not randomly assigned and some confounders are unobserved, causal estimates from regression or matching methods are generally biased. A valid instrument provides a source of variation that affects treatment assignment but does not affect the outcome through any other channel. IV exploits this externally induced variation in treatment to estimate a causal effect.
...
A simple comparison between more educated and less educated individuals may be biased because schooling is associated with unobserved ability and family background. Card uses geographic proximity to college as an instrument for educational attainment.

The identification argument has three components (see next section for more details):

1. **Relevance:** individuals who grow up near a college face lower attendance costs, which may encourage them to obtain more schooling.
2. **Exclusion:** proximity to college must affect future wages only through schooling, not through other channels such as better local employment opportunities.
3. **Exogeneity:** after conditioning on relevant pre-treatment covariates, proximity must be unrelated to unobserved determinants of wages. This could fail, for example, if families with greater unobserved resources systematically choose to live near colleges.

In this setting:

- $Z_i$ indicates whether an individual grew up near a college;
- $T_i$ denotes years of schooling;^[Unlike the binary treatment indicator $D_i$ used elsewhere in this chapter, $T_i$ measures treatment intensity: years of schooling. A binary instrument can shift a treatment that takes more than two values.]
- $Y_i$ denotes log wages;
- $X_i$ contains observed control variables.

Card reports IV estimates of the return to schooling that are roughly 25–60% higher than conventional OLS estimates across his specifications. He also finds that the educational gains associated with college proximity are concentrated among men from less-educated families, for whom nearby access is more likely to change educational attainment.

## The Basic IV Logic and the Key Assumptions

An instrument is usually denoted by $Z_i$. The core IV idea is that $Z_i$ affects treatment status $D_i$, and this instrument-induced variation in treatment is then used to learn about the effect of $D_i$ on the outcome $Y_i$:

$$
Z_i \rightarrow D_i \rightarrow Y_i.
$$

For this logic to identify a causal effect, the instrument must satisfy three conditions at the same time.
...
In practice, this means that changes in the instrument should generate changes in the probability of receiving the treatment, in the case of a binary treatment. Relevance is empirically assessable: we can estimate the first-stage relationship between $D_i$ and $Z_i$ and check whether the instrument significantly predicts treatment even after controlling for $X_i$.

2. The instrument must affect the outcome **only through the treatment**. This is the **exclusion restriction**: changing $Z_i$ must not change $Y_i$ through any channel that bypasses $D_i$.

3. The instrument must also be **exogenous**: possibly after conditioning on pre-treatment covariates, it must be unrelated to unobserved factors that influence the outcome. An instrument can satisfy exclusion and still fail this condition. For example, distance to college might affect earnings only through education, yet remain associated with family background because families choose where to live.

In the linear outcome model, instrument validity requires the following moment condition:

$$
\operatorname{Cov}(Z_i,u_i\mid X_i)=0.
$$

This condition should not be interpreted as the exclusion restriction alone: both alternative causal channels and associations with unobserved outcome determinants can invalidate it.

Unlike relevance, exclusion and exogeneity generally cannot be established from the observed data alone. Their credibility must be justified using institutional knowledge, theory, and a convincing account of how the instrument is generated.

::: {.callout-caution}
### The exclusion restriction

The exclusion restriction is usually the most demanding IV assumption. A strong first stage is not enough: if the instrument affects the outcome through channels other than the treatment, the exclusion restriction is violated and the IV estimate cannot generally be given a causal interpretation.
:::
...
The estimated coefficient $\widehat{\tau}_{\text{2SLS}}$ is the IV estimate obtained by 2SLS. The intuition is simple: rather than relying on all observed variation in treatment, 2SLS isolates the variation associated with the instrument, after accounting for the control variables. If the instrument is valid, this variation can be interpreted as exogenous^[In this context, **exogenous variation** means variation in treatment that is generated by the instrument rather than by factors that also affect the outcome. It can therefore be treated as unrelated to the unobserved determinants of the outcome.] and used to estimate a causal effect.
...
Here, $\rho_1$ captures the effect of the instrument on the outcome, conditional on $X_i$. With a single instrument and a single treatment, the 2SLS estimate can equivalently be written as the ratio between the reduced-form effect and the first-stage effect:
  
  $$ \widehat{\tau}_{\text{2SLS}} = \frac{\widehat{\rho}_1}{\widehat{\pi}_1}. $$
    
This ratio asks: how much does the outcome change relative to the amount of treatment variation induced by the instrument?
...
Another famous IV example is Angrist and Krueger's study of compulsory schooling laws [@angrist1991]. They use quarter of birth as an instrument for years of schooling.
...
This suggests that an additional year of schooling increases wages by about 8.6% for the students whose schooling is affected by the instrument.
...
For a binary instrument and a binary treatment, the LATE interpretation requires four main assumptions.
...
2. The instrument must satisfy the **exclusion restriction**: holding treatment status fixed, changing the instrument must not change the outcome. In other words, the instrument can affect the outcome only by changing the treatment received.
...
When the instrument is as good as randomly assigned without conditioning on covariates, these assumptions identify the LATE through the unconditional Wald ratio:
...
The numerator is the reduced-form effect of the instrument on the outcome. The denominator is the first-stage effect of the instrument on treatment. **Intuitively, the ratio scales the change in the outcome caused by the instrument by the change in treatment participation caused by the instrument.** It therefore asks how much the outcome changes for each additional unit induced into treatment by the instrument. For example, if the instrument increases the outcome by 2 units and raises the probability of treatment by 0.25, the implied LATE is $2/0.25=8$.

If the instrument is as good as randomly assigned only after conditioning on $X_i$, the unconditional ratio above is generally not valid. The outcome and treatment comparisons must then account for those covariates. Conditional Wald ratios identify effects for compliers with given covariate values, where the first stage is nonzero; aggregating these effects requires weights that reflect the target complier population.

### Example: Vietnam Draft Lottery

::: {.content-visible when-format="html"}
![**A gif representation of the IV example**.](../images/iv-draft-lottery-animation.gif)
:::

A classic example with a binary instrument is Angrist's study of the effect of military service on later earnings [@angrist1990]. During the Vietnam War, the United States used a draft lottery to determine who was at risk of being drafted.
...
When using proximity to college as an instrument for years of education, as in the Card [-@card1993] example, college proximity is often introduced as a binary instrument: individuals are classified as living near or far from a college. The instrument therefore defines two groups, but the treatment itself, years of schooling $T_i$, is not binary.

This distinction matters for interpretation. Proximity may induce one person to complete one additional year of schooling and another to complete several additional years. Under suitable exogeneity, exclusion, and monotonicity assumptions, IV identifies a weighted average of the effects of additional years of schooling across the educational margins shifted by the instrument. It is not simply the binary-treatment LATE defined above, nor necessarily the average return to schooling for all students.
...
This adds a further layer to interpretation. The individuals whose education changes when distance increases from 5 to 10 kilometers need not be the same as those affected by a change from 50 to 55 kilometers. The IV estimate may therefore combine responses across different changes in the instrument as well as different educational margins. Its interpretation depends on the instrument variation used and the assumptions governing these responses.
:::

## Interpreting IV Estimates

Interpreting IV estimates requires care. Each IV estimate depends on the instrument, because the instrument determines which units are shifted into treatment.
...
This means that IV estimates should not automatically be interpreted as average effects for everyone. They generally retrieve average causal effects only for the units affected by the instrument.
...
With a binary instrument and a binary treatment, IV identifies the LATE for compliers under the assumptions described above. With non-binary treatments, the interpretation must also account for the treatment margins shifted by the instrument. This makes IV powerful but also subtle: the estimate depends on who is moved by the instrument and how their treatment changes.
