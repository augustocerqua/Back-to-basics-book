...
<!--When a policy affects only one unit, or a small number of aggregate units, constructing a credible counterfactual becomes especially challenging.
-->
Most impact evaluation methods rely on comparisons between many treated and untreated units. This logic is harder to apply when treatment is assigned to a single country, region, city, or municipality. In such settings, no individual untreated unit is likely to provide a fully convincing representation of what would have happened to the treated unit in the absence of the intervention. The **synthetic control method (SCM)** addresses this problem by constructing the counterfactual as a weighted combination of untreated units.
...
![SCM constructs a **synthetic control** by taking a weighted average of untreated donor units. The weights are chosen so that the synthetic control resembles the treated unit before the intervention, in terms of relevant pre-treatment characteristics such as age, wage, and years of education. The treated unit is then compared with this synthetic counterpart after the intervention to estimate the treatment effect.](../images/SCM_in_a_figure_version3.png){width="80%"}
:::
...
The goal is to construct a synthetic unit that closely reproduces the treated unit before the intervention. If the treated unit and the synthetic control follow similar pre-treatment trajectories, especially for the outcome of interest, this provides support for using the synthetic control as an approximation of how the treated unit’s outcome would have evolved in the absence of the intervention.
...
Another possibility is to compare the treated unit with the simple average of all untreated units. This approach can also be problematic, because untreated units may differ substantially from the treated unit in several respects, including their pre-treatment outcome levels and trends. A simple average may therefore provide a poor approximation of what would have happened to the treated unit in the absence of the treatment.
...
In practice, the synthetic control is constructed in a **data-driven** way. The researcher starts from a pool of untreated units and defines the pre-treatment information that should be matched, typically including past values of the outcome variable and, when relevant, other covariates. The weights assigned to the untreated units are then determined by an optimization procedure that searches for the weighted combination that best reproduces the treated unit before the intervention. Units that improve the pre-treatment fit receive positive weights, while units that do not contribute to reproducing the treated unit receive zero or very small weights. The resulting synthetic unit is therefore not selected subjectively, but built from the data to mimic the treated unit before treatment.
...
Including units that are very different from the treated unit can be problematic, even if they help improve the pre-treatment fit. With a very broad or poorly selected donor pool, the algorithm may find a combination of unrelated units that reproduces the treated unit before the intervention only by chance.^[A donor unit can receive a positive weight even if it is individually quite different from the treated unit, because SCM chooses weights for the **combination** of donors rather than evaluating each donor in isolation. Differences in one donor may be offset by differences in others, so that their weighted average closely matches the treated unit.] This is a form of **overfitting**: the synthetic control may look good in the pre-treatment period, but it may not represent a credible counterfactual after the treatment.

For this reason, the donor pool should not be chosen mechanically or by uncritically including all available untreated units. It should be restricted to units that, based on substantive knowledge of the empirical setting, could plausibly have followed a similar trajectory to the treated unit in the absence of the intervention. The goal is not simply to obtain the best possible pre-treatment fit, but to construct a counterfactual that is both empirically accurate and substantively credible.
...
Before the intervention, the treated unit and the synthetic control should display similar values of the outcome and relevant predictors. If the synthetic control cannot reproduce the treated unit before the treatment, it is difficult to trust it as a reliable counterfactual after the treatment.
...
The estimated effect can vary over time. This is particularly useful when the impact of a policy is not immediate but gradually emerges during the post-treatment period, or when the effect is initially strong and then fades over time.
...
1. **The donor pool should be carefully chosen and contain units that are reasonably comparable to the treated unit.** If the treated unit is very different from all available untreated units, no weighted average of them is likely to provide a credible counterfactual. At the same time, the goal is not merely to obtain a good statistical fit, but to construct a synthetic control from donors that are also substantively plausible comparisons.

2. The synthetic control should reproduce the treated unit well before the intervention. A good **pre-treatment fit** is essential: if the synthetic unit does not behave like the treated unit before treatment, it is difficult to believe that it represents what would have happened afterward in the absence of treatment.

3. The pre-treatment period should be sufficiently informative. SCM works best when the treated and untreated units are observed for several periods before the intervention, so that the algorithm has enough information to construct a credible synthetic counterpart.^[With only a few pre-treatment periods, it is relatively easy for the algorithm to find a combination of donor units that fits the treated unit well, potentially even by chance. A close fit becomes more informative when it is sustained over a long pre-treatment period, because reproducing the treated unit’s trajectory across many periods is more demanding and therefore provides stronger support for the credibility of the synthetic control.]
...
- data are available for a sufficiently long pre-treatment period (e.g., at least 10 periods), allowing the credibility of the synthetic control as a counterfactual to be meaningfully assessed;
...
![**A gif representation of the SCM example**.](../images/scm-germany-reunification-animation.gif)
:::
...
Its main strengths are that, among CBCMs, it is the only method in the evaluator’s toolkit that is suitable for estimating causal effects when there is a single treated unit, and that it constructs the synthetic counterfactual in a transparent manner.^[Other CBCMs can in principle be adapted to settings with a single treated unit, but unlike SCM they are not specifically designed to construct a counterfactual for one treated unit.] SCM shows which untreated units contribute to the counterfactual, how much weight they receive, and how well the resulting synthetic unit reproduces the treated unit before the intervention.
...
The synthetic control outcome estimates the untreated potential outcome for unit 1 at time $t$:
...
Here, $\widehat{Y}_{1t}(0)$ estimates what unit 1, the treated unit, would have experienced at time $t$ without the intervention. The hat indicates an estimate, while $(0)$ denotes the absence of treatment. Each donor's observed outcome, $Y_{jt}$, contributes according to its estimated weight, $\widehat{w}_j$. The weights are non-negative and sum to one.
