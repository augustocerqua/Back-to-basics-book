...
There is therefore a subtle trade-off. If we use eventually treated units as controls after they have already been treated, we may contaminate the comparison group with post-treatment outcomes. But if we exclude all eventually treated units from the control group from the beginning, we are conditioning on a future treatment realization. This is conceptually delicate because, in the potential outcomes framework, comparison groups should be defined using pre-treatment information, not by looking ahead at how treatment status will evolve later. In practice, this is why modern staggered-adoption applications should be explicit about when not-yet-treated units enter the comparison group and when they stop being valid controls.
:::
...
In staggered-adoption designs, event-study plots are not based on a single comparison between one treated group and one control group. They are built from many smaller causal comparisons.

The basic building blocks are the group-time treatment effects, denoted by $ATT(g,t)$. Each $ATT(g,t)$ measures the average effect at calendar time $t$ for the cohort of units first treated in period $g$.
...
For example, $\ell=0$ is the first treated period, $\ell=1$ is one period after treatment, and $\ell=-1$ is one period before treatment. An event-study plot reports one estimate for each event-time horizon $\ell$, obtained by aggregating all available $ATT(g,t)$ values such that $t-g=\ell$.

Formally, an event-time effect can be written as
...
where $\mathcal{G}_{\ell}$ is the set of cohorts for which the event-time horizon $\ell$ is observed, and $\omega_{g\ell}$ are aggregation weights, often related to cohort size.

This construction is useful because it summarizes many cohort-specific effects in a single graph. Instead of reporting a separate effect for every cohort and calendar period, the event-study plot shows how effects evolve before and after treatment.
...
For example, suppose one cohort is first treated in period 3 and another in period 4. The cohort treated in period 3 cannot contribute to the estimate at event time $\ell=-4$, because four pre-treatment periods before treatment are not observed for that cohort. As a result, the estimate at $\ell=-4$ is based only on cohorts with enough pre-treatment history. Similarly, long-run post-treatment effects may be based only on early-treated cohorts.
...
![This figure illustrates how event-study coefficients are constructed under staggered treatment adoption. The left panel shows that cohorts receive the treatment in different periods and may differ in the number of treated units. The upper-right panel maps these cohorts into event time: for each value of $k$, the event-study coefficient is obtained by aggregating the available cohort-specific treatment effects, with weights proportional to cohort size. A key point is that event-time coefficients are not always based on the same set of cohorts. In the central part of the event-study window, all cohorts contribute to the aggregate estimate. At the extremes, only some cohorts are observed. As shown in the lower panel, estimates for early leads and late lags are therefore based on fewer treated units and may be less precise, as reflected in wider confidence intervals. The figure also highlights an additional issue: when treatment effects are heterogeneous across cohorts, changes in the estimated event-study profile may reflect both true effect dynamics and changes in composition. For example, the steeper increase from period $+3$ onward may partly depend on the fact that the composition of contributing cohorts changes once Cohort 4 drops out. As an alternative, researchers may restrict event-study plots to the pre- and post-treatment periods in which all cohorts are observed. In this example, this would limit the event-study window to periods from $-2$ to $+2$.](../images/event-study-building-blocks-aggregation_v3.png)
