...
The canonical DiD design compares treated and control units before and after treatment, in its simplest form using two groups and two periods. But many real policy settings are richer. Units may be treated at different times, treatment effects may evolve gradually, and researchers may want to know not only whether a policy had an effect, but also **when** the effect appeared and how it changed over time.

This is the main motivation for **event-study designs**. In this chapter, the term event study refers to modern DiD-based approaches that estimate treatment effects relative to the timing of an event, such as the adoption of a reform, the introduction of a policy, or the occurrence of a shock.^[The term **event study** is also used differently in other fields. In finance, for example, traditional event studies typically measure abnormal stock returns around a specific event by comparing observed returns with the returns predicted by a model of normal market performance. This differs from the DiD-based event-study designs discussed here, which compare outcome dynamics across groups relative to the timing of treatment.]
...
Let $G_i$ denote the first period in which unit $i$ is treated. For never-treated units, let $G_i=\infty$. Let $t$ denote calendar time. The treatment indicator is
...
When $\ell=0$, the unit is in its first treated period; when $\ell=1$, it is one period after treatment onset; and when $\ell=-1$, it is one period before treatment.
...
Throughout this chapter, $ATT^{ES}(\ell)$ denotes an aggregate post-treatment effect at event time $\ell \geq 0$, where the superscript $ES$ stands for **event study**. Pre-treatment diagnostic estimates are denoted separately by $\pi_{\ell}$ for $\ell < -1$, where $\ell=-1$ is used as the reference period.^[The **reference period** is the period against which all event-study estimates are compared. Its coefficient is normalized to zero, so each estimated coefficient measures the difference relative to that period. Using $\ell=-1$ means that the period immediately before treatment serves as the baseline.] The sample estimates are written as $\widehat{ATT}^{ES}(\ell)$ and $\widehat{\pi}_{\ell}$, respectively.

The pre-treatment estimates are useful diagnostics. Since treatment has not yet occurred, they should generally be close to zero, unless units anticipate the treatment or treated and control units were already following different trends.

The post-treatment estimates describe treatment dynamics. The effect may appear immediately, grow over time, fade out, or even change sign.

::: {.callout-note appearance="simple"}
### Event-time plot

The right-hand graph of Figure 7.1 provides a typical example of an event-study plot, showing pre-treatment diagnostic estimates and post-treatment dynamic effects, usually with confidence intervals and a vertical line at the treatment date. This visual representation makes the results particularly immediate and easy to interpret, allowing researchers to assess both pre-treatment patterns and the evolution of treatment effects at a glance. This transparency has likely contributed to the widespread adoption of event-study designs in applied research.
:::
...
This specification is intuitive, but it can be problematic under staggered adoption as we have already seen at the end of [Chapter 6](../chapters/06-difference-in-differences.qmd). To avoid such identification issues, modern event-study methods construct comparisons more carefully as shown below.

## Clean Comparisons

The safest comparisons are usually those between newly treated units and units that are still untreated at the same calendar time, either because they will be treated later or because they remain untreated throughout the analyzed period.
...
The identifying assumption is a modified version of the parallel trends assumption:
...
However, not-yet-treated units can serve as “clean” controls only up to the point at which they receive treatment. After that, their outcomes may reflect treatment effects, and using them as controls can bias ATT estimates in later periods by design.
  ...
  There is therefore a trade-off between using **not-yet-treated units** and restricting the control group to **never-treated units**. Using not-yet-treated units enlarges the pool of available controls and may provide more comparable units, but each of these units can serve as a control only until it becomes treated. As treatment adoption progresses, the composition of the control group therefore changes over time. Restricting the analysis to never-treated units instead provides a stable group that remains unaffected by treatment throughout the analyzed period. However, this group may be smaller and systematically different from the treated units, making the parallel-trends assumption less plausible. Neither choice is inherently preferable: researchers should make clear which units serve as controls at each point in time and assess whether they provide a credible comparison for the treated units.
:::
...
Another approach is to combine matching with DiD. The basic matching DiD estimator, already presented in [Chapter 6](../chapters/06-difference-in-differences.qmd), uses matching to account for observed differences between treated and control units and differencing to account for time-invariant unobserved differences [@heckman1997].

Imai, Kim, and Wang extend this logic to time-series cross-sectional (TSCS) data^[**TSCS data** have essentially the same two-dimensional structure as panel data: multiple units are observed repeatedly over time. The term *TSCS* is especially common in political science, often for aggregate units such as countries, states, or municipalities observed over a relatively long time span. There is no sharp boundary between TSCS and panel data.] through **PanelMatch** [@imai2023], a non-parametric matching approach that provides an alternative way to estimate the dynamic treatment effects typically displayed in event-study analyses. The method constructs matched sets for treated observations using their pre-treatment histories.

The procedure can be summarized in four steps:

1. For each treated unit-period, construct an initial set of eligible controls observed at the same time through exact matching, requiring controls to have the same treatment history over a chosen number of pre-treatment periods and, when appropriate, the same values of selected covariates.^[**Exact matching** requires treated and control observations to have exactly the same values for the variables on which the restriction is imposed. For example, a treated municipality may be compared only with municipalities in the same region or population category. This differs from matching methods that allow controls to be similar, but not identical, on these characteristics.]
2. Refine this initial matched set by selecting or reweighting controls to improve balance in pre-treatment outcomes and covariate histories.
3. Use the resulting matched controls to construct counterfactual outcomes for the treated observations.
4. Estimate treatment effects over different post-treatment horizons.
...
The weights determine how much each contributing cohort matters for the event-time effect. A natural, and generally preferable, choice is to weight cohorts in proportion to their size, so that the aggregate effect reflects the relative number of treated units represented by each cohort. The corresponding sample estimate is denoted by $\widehat{ATT}^{ES}(\ell)$.
...
### Non-significant pre-trends are not evidence of equivalence

Too much emphasis is often placed on whether pre-treatment event-study estimates are statistically significant. **Failure to reject zero is not evidence that departures from parallel trends are negligible.** Wide confidence intervals can include both zero and differences large enough to undermine the causal interpretation. This reflects low statistical power, not necessarily a credible comparison group.

For a stylized example, suppose an employment-rate pre-treatment contrast is estimated at zero, with a 95% confidence interval from -5 to +5 percentage points. It is not statistically significant, but it does not rule out substantial pre-treatment divergence. That uncertainty is particularly concerning if the estimated post-treatment effect is only two percentage points. The comparison is a diagnostic warning, not a direct calculation of post-treatment bias.

Bilinski and Hatfield [-@bilinski2026parallel] propose a non-inferiority/equivalence approach: ask whether the data can rule out violations large enough to matter, using a substantively justified threshold. **The practical lesson is to assess the magnitude and precision of pre-treatment estimates, not simply whether their intervals contain zero**.

Even precise evidence of similar pre-treatment trends cannot establish that untreated trends would have remained parallel after treatment. Institutional reasoning and sensitivity analysis remain essential.
:::
...
![This figure illustrates how event-study estimates are constructed under staggered treatment adoption. The left panel shows that cohorts receive the treatment in different periods and may differ in the number of treated units. The upper-right panel maps these cohorts into event time: at each post-treatment value of $\ell$, the event-study estimate aggregates the available cohort-specific treatment effects, with weights proportional to cohort size. Pre-treatment points are diagnostic placebo estimates. A key point is that event-time estimates are not always based on the same set of cohorts. In the central part of the event-study window, all cohorts contribute to the aggregate estimate. At the extremes, only some cohorts are observed. As shown in the lower panel, estimates for early leads and late lags are therefore based on fewer treated units and may be less precise, as reflected in wider confidence intervals. The figure also highlights an additional issue: when treatment effects are heterogeneous across cohorts, changes in the estimated event-study profile may reflect both true effect dynamics and changes in composition. For example, the steeper increase from period $+3$ onward may partly depend on the fact that the composition of contributing cohorts changes once Cohort 4 drops out. As an alternative, researchers may restrict event-study plots to the pre- and post-treatment periods in which all cohorts are observed. In this example, this would limit the event-study window to periods from $-2$ to $+2$. The point at $\ell=-1$ is the reference period: it is normalized to zero and is not estimated separately.](../images/event-study-building-blocks-aggregation_v5.png)

::: {.callout-note appearance="simple"}
### Application: Follow up on unilateral divorce laws and dynamic effects

A classic application of event-study methods is Wolfers’ analysis of unilateral divorce laws in the United States [@wolfers2006]. We already introduced this application in [Chapter 6](../chapters/06-difference-in-differences.qmd) to illustrate the logic of DiD. Here, we return to the same setting from an **event-study perspective**, focusing on how the effects evolve relative to the timing of policy adoption.

The setting is well suited to an event-study design because U.S. states adopted unilateral divorce laws in different years. This staggered timing makes it possible to compare the evolution of divorce rates before and after adoption across different groups of states. The event-study framework is especially useful because the effect of the reform need not be immediate or constant over time. Divorce rates may respond gradually after the law changes, and the effect may increase, decline, or disappear several years after adoption.
...
- whether treatment is absorbing or can turn on and off;^[An **absorbing treatment** is one that, once adopted, remains in place in all subsequent periods. By contrast, a non-absorbing treatment can switch on and off over time, so the same unit may move between treated and untreated states.]
- whether never-treated and/or not-yet-treated units are used;
- event-time plots with confidence intervals;
- whether anticipation or carryover effects are plausible;^[**Carryover effects** occur when treatment continues to affect outcomes even after it has ended. In this case, a unit that is currently untreated may still be influenced by its past treatment exposure, complicating the definition of a clean control period.]
...
![Generic example of a treatment-variation plot in a setting with staggered adoption and an absorbing treatment.](../images/treatment_variation_plot.png)
...
If the pre-treatment estimates are close to zero and the post-treatment estimates increase gradually, the evidence would be consistent with a positive employment effect that emerges over time. By contrast, if the estimates are already increasing before treatment begins, the credibility of the design would be weakened.
...
The lesson of recent years is that event-study estimates should not be produced mechanically using TWFE regressions. In particular, when treatment adoption is staggered and treatment effects are heterogeneous, TWFE estimators may rely on problematic comparisons and assign implicit weights that complicate the interpretation of the estimated effects.

Modern event-study estimators instead build effects from clearer comparisons, such as treated cohorts versus not-yet-treated or never-treated units, and then aggregate group-time effects transparently. The central questions are: what is the comparison group, what is the target estimand, and how are the dynamic effects aggregated?
